...
/** Default XSD dir for a script: its own folder if it has .xsd files, else ./xsd if present. */
function defaultXsdDir(scriptPath) {
  const dir = path.dirname(path.resolve(scriptPath));
  const hasXsd = (d) => fs.existsSync(d) && fs.readdirSync(d).some(f => f.toLowerCase().endsWith('.xsd'));
  if (hasXsd(dir)) return dir;
  if (hasXsd('xsd')) return 'xsd';
  return dir;
}

module.exports = { loadTypes, parseXsdFile, suggest, XS_TO_JS, defaultXsdDir };
